...
export default function ProjectDetailPage({
  ...
  // Route params are Promises in Next.js 16; `use` unwraps them here.
  ...
  /** Set for the statuses that need confirming before they are applied. */
  ...
  React.useEffect(() => {
    ...
    const load = async () => {
      try {
        // The roster feeds both the member table and the add-member picker,
        // since the project payload only names people it already includes.
        ...
        // The detail endpoint returns taskStats beside the project, not on it.
  ...
  /**
   * The server merges managers and team leads into `members`, but union them
   * anyway so nobody is hidden from the team table if it ever does not.
   */
  const allMemberIds = [
    ...new Set([
      ...project.memberIds,
  ...
  /** Resolves a member id to a person, preferring the populated project data. */
  ...
  /**
   * Routes a status change to the right endpoint.
   *
   * Archiving has its own endpoints rather than going through the status field,
   * and `unarchive` always restores to Active — so moving from Archived to
   * anything else needs a follow-up update.
   */
  ...
  /** Completing or archiving is consequential, so those confirm first. */
